...
import { Bot, FileText, Briefcase, Mail, Sparkles, File } from "lucide-react";

// Templates for different document types
const TEMPLATES = {
  blank: {
    emoji: "📝",
    template: "", // Blank template with no content
  },
  resume: {
    emoji: "📄",
    template: `
                  ...
                  >
                    <div className="flex flex-col items-center">
                      ...
                      <span className="font-semibold capitalize">
                        {key === 'blank' ? 'Blank Document' : key.replace(/([A-Z])/g, " $1").trim()}
                      </span>
                    </div>
                  </div>
                ))}
